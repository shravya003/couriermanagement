...
(1, 'shravs@gmail.com', 'Delay', 'I had ordered 2 weeks ago but its not delivered yet !'),
(2, 'shraddha@gmail.com', 'Good Service', 'Received courier in a good manner.');
...
(1, 2, 'shravs@gmail.com', 'shraddha@gmail.com', 'saaima', 'prakrithi', '6362786223', '6362786224', 'VV Puram,Bangalore,Karnataka', 'Mysuru Road,Bangalore,Karnataka', 2, 001, 'cddd.jpeg', '2021-12-06'),
(2, 1, 'shraddha@gmail.com', 'shravs@gmail.com', 'prakrithi', 'saaima', '6362786224', '6362786223', 'Mysuru Road,Bangalore,Karnataka', 'VV Puram,Bangalore,Karnatak', 5, 002, 'fc.png', '2021-12-21');

-- --------------------------------------------------------
DELIMITER $$
CREATE TRIGGER update_courier
AFTER INSERT ON courier
FOR EACH ROW
BEGIN
    IF STDATE==TDDATE THEN
        UPDATE courier
        SET status = 'Status >>On the way'
        WHERE email= remail;
    ELSE
        UPDATE courier
        SET status = 'Status>>Items delivered'
        WHERE email = remail;
    END IF;
END$$
DELIMITER ;
...
('shravs@gmail.com', '12345', 1),
('shraddha@gmail.com', '12345', 2);
...
BEGIN
    ...
    SELECT COUNT(*) INTO duplicate FROM user WHERE email = NEW.email;
    IF duplicate > 0 THEN
    SIGNAL SQLSTATE '45000'
     //notice:if the email id is registered before it will not respond
    END IF;
END$$
DELIMITER ;
...
(1, 'shravs@gmail.com', 'Shravya', 99999),
(2, 'shraddha@gmail.com', 'Shraddha', 88888);
...
CREATE PROCEDURE delete_courier(IN c_id INT)
BEGIN
    DELETE FROM courier
    WHERE c_id=c_id;
END
...
CREATE PROCEDURE update_courier_details(IN c_id INT, IN semail VARCHAR(50),
 IN remail VARCHAR(50), IN sname VARCHAR(50), IN rname VARCHAR(50),  
 IN sphone VARCHAR(20), IN rphone VARCHAR(20),IN saddress VARCHAR(50),
  IN raddress VARCHAR(50))
BEGIN
    UPDATE courier
    SET semail = semail, remail = remail, sname = sname, rname = rname, sphone = sphone, 
    rphone = rphone, saddress = saddress, raddress = raddress
    WHERE c_id=c_id;
END